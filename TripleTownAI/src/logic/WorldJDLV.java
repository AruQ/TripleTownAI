...
public class WorldJDLV
{
		...
		try
		{
			...
			while (_BUFFERED_HANDLER_findNeighbour.hasMoreModels())
			{
				...
				it.unical.mat.jdlv.program.TypeSolver.loadPredicate(neighbours, "neighbour",
						_temporary_JDLV_MODELfindNeighbour, Cell.class);
				it.unical.mat.jdlv.program.JDLV_Logger.getInstance().logInfoMessage(
	...
	public static java.util.List<Cell> bearAvaibleMovements(Matrix matrix, Cell bear)
	{
		...
		// ---- START - startProgram ----
		it.unical.mat.jdlv.program.JDLV_Logger.getInstance().logInfoMessage(
				"Creation findBearMovements JDLV module.");
		...
		// ---- START - addInMapping ----
		_JDLV_PROGRAM_findBearMovements.addText(it.unical.mat.jdlv.program.TypeSolver
				.getNameTranslation(cells, "matrix"));
		it.unical.mat.jdlv.program.JDLV_Logger.getInstance().logInfoMessage(
				"Add in-mapping 'cells::matrix' in module findBearMovements:"
						+ it.unical.mat.jdlv.program.JDLV_Logger.getInstance().getPrettyCode(
								it.unical.mat.jdlv.program.TypeSolver.getNameTranslation(cells,
										"matrix"), 0));
		...
		// ---- START - addInMapping ----
		_JDLV_PROGRAM_findBearMovements.addText(it.unical.mat.jdlv.program.TypeSolver
				.getNameTranslation(bear, "bear"));
		it.unical.mat.jdlv.program.JDLV_Logger.getInstance().logInfoMessage(
				"Add in-mapping 'bear::bear' in module findBearMovements:"
						+ it.unical.mat.jdlv.program.JDLV_Logger.getInstance().getPrettyCode(
								it.unical.mat.jdlv.program.TypeSolver.getNameTranslation(bear,
										"bear"), 0));

		// ---- END - addInMapping ----
		it.unical.mat.jdlv.program.JDLV_Logger.getInstance().logInfoMessage(
				"Add out-mapping 'avaibleMovements::avaibleMovement' in module findBearMovements.");

		// ---- START - prepareJDLVCall ----
		try
		{

			_JDLV_INVOCATION_findBearMovements = it.unical.mat.wrapper.DLVWrapper.getInstance()
					.createInvocation(
							it.unical.mat.jdlv.util.JdlvProperties.getInstance()
									.getDlvExecutablePath());
			...
			_JDLV_PROGRAM_findBearMovements
					.addText("adjacent(X, Y, X1, Y1) :- adjacent(X1, Y1, X, Y)." + '\n');
			_JDLV_PROGRAM_findBearMovements
					.addText("avaibleMovement(X, Y, \"Empty\") :- bear(X1, Y1, _), adjacent(X, Y, X1, Y1), matrix(X, Y, \"Empty\")." + '\n');
			...
			it.unical.mat.wrapper.ModelBufferedHandler _BUFFERED_HANDLER_findBearMovements = new it.unical.mat.wrapper.ModelBufferedHandler(
					_JDLV_INVOCATION_findBearMovements);
			it.unical.mat.jdlv.program.JDLV_Logger.getInstance().logInfoMessage(
					"Start execution findBearMovements program: executablePath='"
							+ it.unical.mat.jdlv.util.JdlvProperties.getInstance()
									.getDlvExecutablePath()
							+ "', options='"
							+ _JDLV_INVOCATION_findBearMovements.getOptionsString()
							+ "'"
							+ '\n'
							+ "Code execution: "
							+ it.unical.mat.jdlv.program.JDLV_Logger.getInstance().getPrettyCode(
									_JDLV_INVOCATION_findBearMovements.getInputProgram()
											.getCompleteText(), 0));
			...
			while (_BUFFERED_HANDLER_findBearMovements.hasMoreModels())
			{
				it.unical.mat.wrapper.Model _temporary_JDLV_MODELfindBearMovements = _BUFFERED_HANDLER_findBearMovements
						.nextModel();
				it.unical.mat.jdlv.program.TypeSolver.loadPredicate(avaibleMovements,
						"avaibleMovement", _temporary_JDLV_MODELfindBearMovements, Cell.class);
				it.unical.mat.jdlv.program.JDLV_Logger.getInstance().logInfoMessage(
						"Process new answer_set"
								+ '\n'
								+ "Results:"
			...
			{
				throw new java.lang.RuntimeException(_JDLV_INVOCATION_findBearMovements.getErrors()
						.get(0).getText());
			}
		} catch (java.lang.Throwable _JDLV_EXCEPTION_findBearMovements)
		{
			it.unical.mat.jdlv.program.JDLV_Logger.getInstance().logErrorMessage(
					_JDLV_EXCEPTION_findBearMovements.getMessage());
		}
		_JDLV_PROGRAM_findBearMovements.cleanText();
